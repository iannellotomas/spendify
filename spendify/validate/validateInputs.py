...
def validateOptionInput(option, max, min=1):
    if isNumeric(option):
        ...
        if parsedOption > max or parsedOption < min:
            showError("La opción ingresada está fuera de rango")
        else:
            return True
    else:
        showError("La opción ingresada no es un número")
    ...
    while not isValid:
        ...
        if isNumber:
            if isNumeric(data):
                ...
                showError("Tenés que ingresar un número en el campo de " + variable)
                continue
    ...
    while not isValidOption:
        ...
        if isNumeric(data):
            ...
            if 1 <= data <= len(array):
                ...
                showError(f"Ingresá un número dentro del rango (1-{len(array)})")
        else:
            showError(f"Tenés que ingresar una de las opciones para {variable}")
